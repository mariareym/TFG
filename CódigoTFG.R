  ...
  BASE <- BASE %>%
    ...
    y_hpresult <- hpfilter(BASE$ln_PIBr, freq=1400)
    g_hpresult <- hpfilter(BASE$ln_gasto, freq=1400)
    gr_hpresult <- hpfilter(BASE$ln_gastor, freq=1400)
  ...
  #Parte7 - Análisis
    ...
    adf_ciclo_gasto <- adf.test(BASE$ciclo_gasto)
    print(adf_ciclo_gasto)
    ...
    coeftest(modelo4s, vcov = NeweyWest(modelo4s, lag = 4))
    ...
    stargazer(modelo0, modelo1, modelo2, modelo3, modelo4, se = list(sqrt(diag(vcov0)), sqrt(diag(vcov1)), sqrt(diag(vcov3)), sqrt(diag(vcov4))),type = "latex", out = "tablamodelos.tex")
    stargazer(modelo0s, modelo1s, modelo2s, modelo3s, modelo4s, se = list(sqrt(diag(vcov0s)), sqrt(diag(vcov1s)), sqrt(diag(vcov3s)), sqrt(diag(vcov4s))),type = "latex", out = "tablamodelos.tex")